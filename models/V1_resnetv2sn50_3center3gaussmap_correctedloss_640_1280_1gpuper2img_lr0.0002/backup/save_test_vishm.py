...
def val(r, name, log=None):
    ...
    for i, data in enumerate(testloader, 0):
        ...
        dtboxes = parse_det_offset(r, pos.cpu().numpy(), height.cpu().numpy(), offset.cpu().numpy(), config.size_test,
                                 score=0.1, down=4, nms_thresh=0.3)
        img_gt_boxs = gtboxes[i]
        img_gt_boxs = np.array(img_gt_boxs)


        box_hm_img = draw_bbox.draw_boxes_bbox_with_hm(merge_all, dtboxes, line_thick=2, line_color='blue')
        gtanddt_box_hm_img = draw_bbox.draw_boxes_bbox_with_dtandgt_hm(box_hm_img, img_gt_boxs, line_thick=2, line_color='white')
        ...
        dtandgt_basename = base_path + str(i) + "dtandgt_hm_bbox" + ".png"
        cv2.imwrite(hm_basename, box_hm_img)
        cv2.imwrite(ori_basename, box_ori_img)
        cv2.imwrite(dtandgt_basename, gtanddt_box_hm_img)
