...
version = 'V22_%s_headandfull2center2gaussmap_%d_%d_%dgpuper%dimg_lr%s'%(net.resnetType, config.size_train[0], config.size_train[1],len(config.gpu_ids),config.onegpu,config.init_lr)
# To continue training
resume_from = ''
resume_from = './models/%s/ckpt/ACSP_105.pth.tea'%version
begin_epoch_num = 0
...
def criterion(output, label): #lebel 0(gausshm, mask, hm)  1(logheigh, mask)  2(height-Y offset, width-X offset, mask)
    print("torch.unsqueeze(output[0][:, 0], dim=1):", torch.unsqueeze(output[0][:, 0], dim=1).shape)
    print("label[0][:, 0], dim=1):", label[0][:, 0].shape)
