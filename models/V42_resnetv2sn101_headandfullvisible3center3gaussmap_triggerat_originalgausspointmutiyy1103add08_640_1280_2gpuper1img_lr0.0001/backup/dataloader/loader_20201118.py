...
class CityPersons(Dataset):
    ...
    def calc_gt_center(self, v_gts, gts, igs, radius=2, stride=4):
        ...
        pos_map = np.zeros((2, 3, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        ...
        if len(gts) > 0:
            ...
            for ind in range(len(gts)):
                ...
                #reedit pos_map-----------------------------------------------------------------------------------------------------
                # gau_map_mean = np.ones(gau_map.shape) * np.mean(gau_map)
                # # cv2.waitKey(0)
                # pos_map[0, 0, y1:y2, x1:x2] = np.maximum(pos_map[1, 0, y1:y2, x1:x2], gau_map_mean)  # gauss map
                # gau_map1 = np.minimum((pos_map[0, 0, yy11:yy12, xx11:xx12] + gau_map1), 1)  # gauss map
                pos_map[0, 0, yy11:yy12, xx11:xx12] = np.maximum(pos_map[0, 0, yy11:yy12, xx11:xx12], gau_map1)  # gauss map
