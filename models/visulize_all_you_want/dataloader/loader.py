...
class CityPersons(Dataset):
    ...
    def calc_gt_center(self, v_gts, gts, igs, radius=2, stride=4):
        ...
        def gaussian_point(kernel, u):
            sigma = ((kernel-1) * 0.5 - 1) * 0.1 + 0.8
            s = 2*(sigma**2)
            dx = np.exp(-np.square(np.arange(kernel) - int(u)) / s)
            return np.reshape(dx, (-1, 1))

        scale_map = np.zeros((3, 2, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        offset_map = np.zeros((3, 3, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        #pse_map[0]->full, pse_map[1]->top, pse_map[2]->center, pse_map[3]->bottom, pse_map[4]->visable
        pos_map = np.zeros((4, 3, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        ...
        if len(gts) > 0:
            ...
            for ind in range(len(gts)):
                ...
                dx = gaussian_point(x2 - x1, 0.5*(x2 - x1))
                dy = gaussian_point(y2 - y1, 0.5*(y2 - y1))
                gau_map = np.multiply(dy, np.transpose(dx))

                w, h = x2 - x1, y2 - y1
                xx11, yy11, xx12, yy12 = int(x1), int(y1), int(x2), int(y1+0.16*h)
                ...
                dxx0 = gaussian_point(x2 - x1, 0.5 * (x2 - x1))  # dx = gaussian(w)
                dyy0 = gaussian_point(y2 - y1, 0.1 * (y2 - y1))  # dy = gaussian(h)
                gau_map0 = np.multiply(dyy0, np.transpose(dxx0))  # h*w

                # reedit pos_map-----------------------------------------------------------------------------------------------------
                pos_map[0, 0, y1:y2, x1:x2] = np.maximum(pos_map[0, 0, y1:y2, x1:x2], gau_map0)  # gauss map
                pos_map[0, 1, y1:y2, x1:x2] = 1  # 1-mask map
                pos_map[0, 2, c_y, c_x] = 1  # center map


                # pos_map[1, 0, yy21:yy22, xx21:xx22] = np.maximum(pos_map[1, 0, yy21:yy22, xx21:xx22], gau_map2)  # gauss map
                # pos_map[1, 1, yy21:yy22, xx21:xx22] = 1  # gauss map
                # pos_map[1, 2, c_y2, c_x2] = 1  # center map
                #
                # pos_map[2, 0, yy31:yy32, xx31:xx32] = np.maximum(pos_map[2, 0, yy31:yy32, xx31:xx32], gau_map3)  # gauss map
                # pos_map[2, 1, yy31:yy32, xx31:xx32] = 1  # gauss map
                # pos_map[2, 2, c_y2, c_x2] = 1  # center map
                ...
                # pos_map[1, 0, v_y1:v_y2, v_x1:v_x2] = np.maximum(pos_map[1, 0, v_y1:v_y2, v_x1:v_x2], v_gau_map)  # gauss map
                # pos_map[1, 1, v_y1:v_y2, v_x1:v_x2] = 1  # gauss map
                # pos_map[1, 2, v_c_y, v_c_x] = 1  # center map
                #end add visible box
                #add full box
                pos_map[1, 0, y1:y2, x1:x2] = np.maximum(pos_map[1, 0, y1:y2, x1:x2], gau_map)  # gauss map
                pos_map[1, 1, y1:y2, x1:x2] = 1  # 1-mask map
                pos_map[1, 2, c_y, c_x] = 1  # center map


                #end add full box\

                dxx3 = gaussian_point(x2 - x1, 0.5 * (x2 - x1))  # dx = gaussian(w)
                dyy3 = gaussian_point(y2 - y1, 0.9 * (y2 - y1))  # dy = gaussian(h)
                gau_map3 = np.multiply(dyy3, np.transpose(dxx3))  # h*w

                # reedit pos_map-----------------------------------------------------------------------------------------------------
                pos_map[2, 0, y1:y2, x1:x2] = np.maximum(pos_map[2, 0, y1:y2, x1:x2], gau_map3)  # gauss map
                pos_map[2, 1, y1:y2, x1:x2] = 1  # 1-mask map
                pos_map[2, 2, c_y, c_x] = 1  # center map

                dxx4 = gaussian(x2 - x1)  # dx = gaussian(w)
                dyy4 = gaussian(y2 - y1)  # dy = gaussian(h)
                gau_map4 = np.multiply(dyy4, np.transpose(dxx4))  # h*w
                pos_map[3, 0, y1:y2, x1:x2] = np.maximum(pos_map[3, 0, y1:y2, x1:x2], gau_map4)  # gauss map
                pos_map[3, 1, y1:y2, x1:x2] = 1  # 1-mask map
                pos_map[3, 2, c_y, c_x] = 1  # center map

                # reedit scale_map-----------------------------------------------------------------------------------------------------
                scale_map[0, 0, c_y1 - radius:c_y1 + radius + 1, c_x1 - radius:c_x1 + radius + 1] = np.log(gts[ind, 3] - gts[ind, 1])  # log value of height
                scale_map[0, 1, c_y1-radius:c_y1+radius+1, c_x1-radius:c_x1+radius+1] = 1  # 1-mask

                # scale_map[1, 0, c_y2-radius:c_y2+radius+1, c_x2-radius:c_x2+radius+1] = np.log(gts[ind, 3] - gts[ind, 1])  # log value of height
                # scale_map[1, 1, c_y2-radius:c_y2+radius+1, c_x2-radius:c_x2+radius+1] = 1  # 1-mask
                #
                # scale_map[2, 0, c_y3-radius:c_y3+radius+1, c_x3-radius:c_x3+radius+1] = np.log(gts[ind, 3] - gts[ind, 1])  # log value of height
                # scale_map[2, 1, c_y3-radius:c_y3+radius+1, c_x3-radius:c_x3+radius+1] = 1  # 1-mask

                # scale_map[1, 0, v_c_y - radius:v_c_y + radius + 1, v_c_x - radius:c_x + radius + 1] = np.log(gts[ind, 3] - gts[ind, 1])  # log value of height
                # scale_map[1, 1, v_c_y - radius:v_c_y + radius + 1, v_c_x - radius:c_x + radius + 1] = 1
                scale_map[1, 0, c_y - radius:c_y + radius + 1, c_x - radius:c_x + radius + 1] = np.log(gts[ind, 3] - gts[ind, 1])  # log value of height
                scale_map[1, 1, c_y - radius:c_y + radius + 1, c_x - radius:c_x + radius + 1] = 1  # 1-mask
                scale_map[2, 0, c_y3 - radius:c_y3 + radius + 1, c_x3 - radius:c_x3 + radius + 1] = np.log(gts[ind, 3] - gts[ind, 1])  # log value of height
                scale_map[2, 1, c_y3 - radius:c_y3 + radius + 1, c_x3 - radius:c_x3 + radius + 1] = 1  # 1-mask

                # reedit offset_map-----------------------------------------------------------------------------------------------------
                offset_map[0, 0, c_y1, c_x1] = (yy11 + yy12) / 2 - c_y1 - 0.5  # height-Y offset
                offset_map[0, 1, c_y1, c_x1] = (xx11+xx12) / 2 - c_x1 - 0.5  # width-X offset
                offset_map[0, 2, c_y1, c_x1] = 1  # 1-mask

                # offset_map[1, 0, c_y2, c_x2] = (yy21+yy22) / 2 - c_y2 - 0.5  # height-Y offset
                # offset_map[1, 1, c_y2, c_x2] = (xx21+xx22) / 2 - c_x2 - 0.5  # width-X offset
                # offset_map[1, 2, c_y2, c_x2] = 1  # 1-mask
                #
                # offset_map[2, 0, c_y3, c_x3] = (yy31+yy32) / 2 - c_y3 - 0.5  # height-Y offset
                # offset_map[2, 1, c_y3, c_x3] = (xx31+xx32) / 2 - c_x3 - 0.5  # width-X offset
                # offset_map[2, 2, c_y3, c_x3] = 1  # 1-mask

                offset_map[1, 0, c_y, c_x] = (y1+y2) / 2 - c_y - 0.5  # height-Y offset
                offset_map[1, 1, c_y, c_x] = (x1+x2) / 2 - c_x - 0.5  # width-X offset
                offset_map[1, 2, c_y, c_x] = 1  # 1-mask

                offset_map[2, 0, c_y3, c_x3] = (gts[ind, 1] + gts[ind, 3]) / 2 - c_y3 - 0.5  # height-Y offset
                offset_map[2, 1, c_y3, c_x3] = (gts[ind, 0] + gts[ind, 2]) / 2 - c_x3 - 0.5  # width-X offset
                offset_map[2, 2, c_y3, c_x3] = 1  # 1-mask
