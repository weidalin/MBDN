...
#     boxes = []
#     for tcw in range(3):
#         pos = np.squeeze(poses[:, tcw])
#         height = np.squeeze(heights[:, tcw])
#         offset_y = np.squeeze(offsets[:, 2*tcw, :, :])
#         offset_x = np.squeeze(offsets[:, 2*tcw+1, :, :])
...
#                 boxes.append([x1, y1, min(x1 + w, size[1]), min(y1 + h, size[0]), s])  # xyxy
#                 x1, y1 = max(0, (x_c[i] + o_x + 0.5) * down - w / 2), max(0, (y_c[i] + o_y + 0.5) * down - h / 2)
#                 boxes.append([x1, y1, min(x1 + w, size[1]), min(y1 + h, size[0]), s])
#                 #do nms per loop
#                 # boxes = np.asarray(boxes, dtype=np.float32)
#                 # keep = nms(boxes, nms_thresh, usegpu=False, gpu_id=0)
#                 # boxes = boxes[keep, :]
#                 # boxes = list(boxes)
#     if len(boxes)>0:
#         boxes = np.asarray(boxes, dtype=np.float32)
...
def parse_det_offset(r, poses, heights, offsets, size, score=0.1, down=4, nms_thresh=0.3):
    ...
    boxs = []
    for tcw in range(2):
        pos = poses[tcw]#(256, 512)
        height = heights[tcw]#(256, 512)
        offset_y = offsets[0, 0+2*tcw, :, :]#(256, 512)
        offset_x = offsets[0, 1+2*tcw, :, :]#(256, 512)
        y_c, x_c = np.where(pos > score)
        if len(y_c) > 0:
            for i in range(len(y_c)):
                h = np.exp(height[y_c[i], x_c[i]]) * down
                ...
                if tcw == 0:
                    ...
                    x1, y1 = max(0, (x_c[i] + o_x + 0.5) * down - w / 2), max(0, (y_c[i] + o_y + 0.5) * down - h / 2)
                boxs.append([x1, y1, min(x1 + w, size[1]), min(y1 + h, size[0]), s])  # xyxy
