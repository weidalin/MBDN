...
version = 'V30_%s_headcenterbottom3center3gaussmap_fullheadbottomcenter_muti01add08016height_%d_%d_%dgpuper%dimg_lr%s'%(net.resnetType, config.size_train[0], config.size_train[1],len(config.gpu_ids),config.onegpu,config.init_lr)
...
def criterion(output, label): #lebel 0(gausshm, mask, hm)  1(logheigh, mask)  2(height-Y offset, width-X offset, mask)
    ...
    # cls_loss_3 = center(torch.unsqueeze(output[0][:, 2], dim=1), label[0][:, 2])
    v_cls_loss = center(torch.unsqueeze(output[0][:, 2], dim=1), label[0][:, 2])
    # cls_loss_refine = center(torch.unsqueeze(output[0][:, 4], dim=1), label[0][:, 4])  # cls_refine

    reg_loss_1 = height(torch.unsqueeze(output[1][:, 0], dim=1), label[1][:, 0])
    reg_loss_2 = height(torch.unsqueeze(output[1][:, 1], dim=1), label[1][:, 1])
    # reg_loss_3 = height(torch.unsqueeze(output[1][:, 2], dim=1), label[1][:, 2])
    v_reg_loss = height(torch.unsqueeze(output[1][:, 2], dim=1), label[1][:, 2])

    off_loss_1 = offset(torch.unsqueeze(output[2][:, 0], dim=1), label[2][:, 0])
    off_loss_2 = offset(torch.unsqueeze(output[2][:, 1], dim=1), label[2][:, 1])
    # off_loss_3 = offset(torch.unsqueeze(output[2][:, 2], dim=1), label[2][:, 2])
    v_off_loss = offset(torch.unsqueeze(output[2][:, 2], dim=1), label[2][:, 2])


    cls_loss = cls_loss_1 + cls_loss_2 + v_cls_loss
    reg_loss = reg_loss_1 + reg_loss_2 + v_reg_loss
    off_loss = off_loss_1 + off_loss_2 + v_off_loss

    # cls_loss = center(output[0], label[0])
    # reg_loss = height(output[1], label[1])
    # off_loss = offset(output[2], label[2])
    # print("cls_loss_refine:", cls_loss_refine.item())
