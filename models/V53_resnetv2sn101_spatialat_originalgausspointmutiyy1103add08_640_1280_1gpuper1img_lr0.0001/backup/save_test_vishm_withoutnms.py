...
def val(r, name, log=None):
    base_path = "/mnt/D0D8D177D8D15C72/cityperson_visualize_heatmap/%s_withoutnms_epoch%s/" % (
    name[name.find('V'): name.find('_')], name[name.rfind('_'): name.find(".pth")])
    base_path_topcenterbottomvis = base_path + '/topcenterbottomvis/'
    if not os.path.exists(base_path):
        os.mkdir(base_path)
    if not os.path.exists(base_path_topcenterbottomvis):
        os.mkdir(base_path_topcenterbottomvis)
    ...
    for i, data in enumerate(testloader, 0):
        ...
        with torch.no_grad():
            pos, height, offset, pos_refine = net(inputs)
        ...
        # box_hm_img = draw_bbox.draw_boxes_bbox_with_channel_nms(merge_all, dtboxes, line_thick=1, line_color='blue')

        # box_hm_img = draw_bbox.draw_boxes_bbox_with_dtandgt_hm(box_hm_img, img_gt_boxs, line_thick=2, line_color='white')
        box_ori_img = draw_bbox.draw_boxes_bbox_with_channel_nms(original_img, dtboxes, line_thick=1, line_color='blue')
        ...
        #plot refine_pos_______________________________________________________________
        vis_cla_pos = pos_refine[0].permute(1, 2, 0).cpu().numpy()  # torch.Size([256, 512, 2])
        vis_cla_pos = cv2.resize(vis_cla_pos, (2048, 1024))  # (1024, 2048, 5)
        vis_cla_pos = vis_cla_pos.transpose(2, 0, 1)
        original_img = data[0].permute(1, 2, 0).cpu().numpy()  # torch.Size([1024, 2048, 3])
        original_img = cv2.normalize(original_img, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
        ...
        #     basename = base_path + str(i) + "_" + loc_type[j] + "_refine.png"
        #     print(basename)
        #     cv2.imwrite(basename, merge)
        gray_img = vis_cla_pos.max(axis=0)
        heatmap = np.uint8(255 * gray_img)
        heatmap_img = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        merge_all = cv2.addWeighted(original_img, 0.4, heatmap_img, 0.7, 0)
        basename = base_path + str(i) + "_" + loc_type[-1] + "_refine.png"
        print(basename)
        # cv2.imwrite(basename, merge_all)

        # plot boxes----------------------------------
        dtboxes = parse_det_offset(r, pos_refine.cpu().numpy(), height.cpu().numpy(), offset.cpu().numpy(), config.size_test,
                                   score=0.3, down=4, nms_thresh=0.5)
        ...
        # box_hm_img = draw_bbox.draw_boxes_bbox_with_dtandgt_hm(box_hm_img, img_gt_boxs, line_thick=2, line_color='white')
        box_ori_img = draw_bbox.draw_boxes_bbox_with_channel_nms(original_img, dtboxes, line_thick=1, line_color='blue')
        hm_basename = base_path + str(i) + "_hm_bbox" + "_refine.png"
        ori_basename = base_path + str(i) + "_ori_bbox" + "_refine.png"
        # cv2.imwrite(hm_basename, box_hm_img)
        cv2.imwrite(ori_basename, box_ori_img)
        print("end %d visualize refine pos" % i)
        # end add visulize ------------------------------------------------------------------------------


# or Val your own model
version = 'V47_resnetv2sn101_headandfullvisiblebottom4center4gaussmap_triggerat_refineloss_pretrainfrom_best_originalgausspointmutiyy1103add08_640_1280_2gpuper1img_lr0.0002'
