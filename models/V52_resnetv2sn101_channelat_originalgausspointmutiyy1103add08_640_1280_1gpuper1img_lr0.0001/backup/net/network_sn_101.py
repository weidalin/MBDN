...
class ACSPNet(nn.Module):
    def __init__(self):
        ...
        # self.spatial_att_att = SpatialAttention()
        self.channel_att = ChannelAttention(3)
    ...
    def forward(self, x):#torch.Size([1, 3, 640, 1280])
        ...
        # x_cls_feat = self.trigger_att(x_cls_feat) # TriggerAttention
        # x_cls_feat = x_cls_feat * self.spatial_att(x_cls_feat) # SpatialAttention
        x_cls_feat = x_cls_feat * self.channel_att(x_cls_feat) # ChannelAttention
...
class TriggerAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(5, 2, kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        a = x[:, :2]
        b = torch.unsqueeze(x[:, 2], dim=0)
        tmp_feat = torch.cos(a - b)
        tmp_feat = torch.cat([tmp_feat, tmp_feat, b], dim=1)
        x = self.conv1(tmp_feat)
        sig = self.sigmoid(x)
        a = a + a * sig
