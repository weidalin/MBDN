...
class CityPersons(Dataset):
    ...
    def calc_gt_center(self, v_gts, gts, igs, radius=2, stride=4):
        ...
        def gaussian_point(kernel, u, delta=0.15):
            sigma = ((kernel-1) * 0.5 - 1) * delta + 0.8
            s = 2*(sigma**2)
            dx = np.exp(-np.square(np.arange(kernel) - int(u)) / s)
            return np.reshape(dx, (-1, 1))

        scale_map = np.zeros((2, 2, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        offset_map = np.zeros((2, 3, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        #pse_map[0]->full, pse_map[1]->top, pse_map[2]->center, pse_map[3]->bottom, pse_map[4]->visable
        pos_map = np.zeros((3, 3, int(self.config.size_train[0] / stride), int(self.config.size_train[1] / stride)))
        ...
        if len(gts) > 0:
            ...
            for ind in range(len(gts)):
                ...
                v_dx = gaussian_point(v_x2 - v_x1,u=(v_x2 - v_x1)*0.5, delta=0.2)
                v_dy = gaussian_point(v_y2 - v_y1,u=(v_y2-v_y1)*0.5,  delta=0.2)
                ...
                # reedit pos_map-----------------------------------------------------------------------------------------------------
                pos_map[0, 0, yy11:yy12, xx11:xx12] = np.maximum(pos_map[0, 0, yy11:yy12, xx11:xx12],
                                                                 gau_map1)  # gauss map
                pos_map[0, 1, yy11:yy12, xx11:xx12] = 1  # gauss map
                pos_map[0, 2, c_y1, c_x1] = 1  # center map
                ...
                #end add full box
                # add visible box
                # pos_map[2, 0, v_y1:v_y2, v_x1:v_x2] = np.maximum(pos_map[2, 0, v_y1:v_y2, v_x1:v_x2],v_gau_map)  # gauss map
                # pos_map[2, 1, v_y1:v_y2, v_x1:v_x2] = 1  # 1-mask map
                # pos_map[2, 2, v_c_y, v_c_y] = 1  # center map
                # end add visible box
                ...
                offset_map[1, 0, c_y, c_x] = (gts[ind, 1] + gts[ind, 3]) / 2 - c_y - 0.5  # height-Y offset
                offset_map[1, 1, c_y, c_x] = (gts[ind, 0] + gts[ind, 2]) / 2 - c_x - 0.5  # width-X offset
