...
def resnetv2sn50(pretrained = True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load('./data/pretrained/ResNet50v2+SN(8,32)-77.57.pth')
...
def resnetv2sn101(pretrained = True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load('./data/pretrained/ResNet101v2+SN(8,32).pth')
        wrong = list(checkpoint['state_dict'].keys())
        right = []
        for i in range(len(wrong)):
            right.append(wrong[i][7:])
