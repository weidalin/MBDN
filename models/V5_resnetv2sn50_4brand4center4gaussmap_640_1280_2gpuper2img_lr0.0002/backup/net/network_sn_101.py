...
class ACSPNet(nn.Module):
    ...
    def forward(self, x):#torch.Size([1, 3, 640, 1280])
        ...
        x_cls1, x_reg1, x_off1, x_cls_feat1 =  self.brand1(cat)
        x_cls2, x_reg2, x_off2, x_cls_feat2 =  self.brand2(cat)
        x_cls3, x_reg3, x_off3,x_cls_feat3 =  self.brand3(cat)
        v_cls4, _, _ , v_cls_feat= self.v_brand(cat)

        # x_cls_feat = torch.cat([x_cls_feat1, x_cls_feat2, x_cls_feat3, v_cls_feat])
        # trigger = self.trigger(x_cls_feat)



        x_cls = torch.cat([x_cls1,x_cls2,x_cls3, v_cls4], dim=1)
        # x_cls = x_cls * trigger

        
        x_reg = torch.cat([x_reg1, x_reg2, x_reg3], dim=1)
        x_off = torch.cat([x_off1, x_off2, x_off3], dim=1)
...
class Brand(nn.Module):
    ...
    def forward(self, x):  # 定义前向传播
        ...
        return x_cls, x_reg, x_off, x_cls_feat
